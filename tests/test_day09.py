...
class Day01TestSuite(unittest.TestCase):
    ...
    def test_part1(self, filename, expected):
        f = open(filename)
        self.assertEqual(day09.part1(f.read().splitlines()), expected)

    @parameterized.expand([("data/day09a.txt", 1134), ("data/day09.txt", 1564640)])
    def test_part2(self, filename, expected):
        f = open(filename)
        self.assertEqual(day09.part2(f.read().splitlines()), expected)
